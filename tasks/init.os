...
Процедура ИнициализацияОкружения()
	...
	ИмяФайлаНастройки = СтрШаблон("./tools/JSON/env.json.build", Аргументы.Команда);
	...
	СтрокаВыполнения = "runner init-dev --src ./src/ --ibconnection " + СтрокаПодключения;
	ИсполнитьКоманду(СтрокаВыполнения);

	СтрокаВыполнения = "runner init-dev --dev --src ./src/ --ibconnection /F./build/ibservice --nocacheuse";
	ИсполнитьКоманду(СтрокаВыполнения);
	...
	Если Не Бинарники1СХранятсяРядомСИсходниками Тогда
		...
		СоздатьКаталог(СтрШаблон("%tests", КаталогСборки));
